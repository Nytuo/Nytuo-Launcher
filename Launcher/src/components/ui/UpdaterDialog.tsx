...
export default function UpdaterModal() {
  ...
  useEffect(() => {
    const unlistenAvailable = listen<UpdateInfo>('updater-update-available', (event) => {
          ...
          <DialogHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <DialogTitle>
                  {phase === 'done' && !updateInfo ? 'Up to date' : 'Update Available'}
                </DialogTitle>
              </div>
            </div>
            ...
            {updateInfo && (
              <DialogDescription asChild>
                <div className="flex items-center gap-1.5 text-sm">
                  <span className="text-muted-foreground">v{updateInfo.current_version}</span>
                  <span className="text-muted-foreground">→</span>
                  <span className="font-semibold text-foreground">v{updateInfo.version}</span>
                  {updateInfo.date && (
          ...
          <div className="flex flex-col gap-4">
            ...
            {(phase === 'idle' || phase === 'error') && (
              <div className="flex flex-col gap-2">
                {updateInfo && (
                  <Button onClick={handleInstall} className="w-full gap-2" disabled={checking}>
                ...
                <div className="flex gap-2">
                  <Button variant="outline" className="flex-1 gap-1.5" onClick={handleOpenReleases}>
