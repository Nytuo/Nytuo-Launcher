...
fn build_updater(app: &AppHandle) -> Result<tauri_plugin_updater::Updater, String> {
    ...
    if let Ok(cfg) = serde_json::from_str::<Value>(include_str!("../../tauri.conf.json")) {
            ...
            .filter(|s| !s.is_empty())
        {
            let url = url::Url::parse(endpoint)
                .map_err(|e| format!("Invalid updater endpoint in config: {e}"))?;
            builder = builder
                .endpoints(vec![url])
                .map_err(|e| format!("Failed to set endpoint: {e}"))?;
            tracing::info!("[updater] Using config endpoint: {}", endpoint);
        }

        if let Some(pubkey) = cfg
            .get("plugins")
            .and_then(|p| p.get("updater"))
            ...
            .filter(|s| !s.is_empty())
        {
            ...
            tracing::info!("[updater] Using config pubkey override");
        }
    }
...
pub async fn open_releases_page(app: AppHandle) -> Result<(), String> {
    ...
    app.opener()
        .open_url(
            "https://github.com/Nytuo/Nytuo-Launcher/releases",
            None::<&str>,
        )
